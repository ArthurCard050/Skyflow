...
import { Clock, MessageSquare, AlertCircle, CheckCircle, Lock, MoreHorizontal, Layers } from 'lucide-react';
...
export const KanbanCard: React.FC<KanbanCardProps> = ({ post, onClick, isDragDisabled }) => {
  const {
    ...
    id: post.id,
    data: { post },
    disabled: isDragDisabled || post.status === 'approved' || post.status === 'published',
  });
  ...
  // SLA Calculation: If pending for > 3 days
  const isSLAOverdue = () => {
  ...
  return (
    <div
      ...
      className={cn(
        "bg-white/70 dark:bg-gray-800/70 backdrop-blur-md p-3 rounded-xl border border-white/40 dark:border-gray-700/50 shadow-sm hover:shadow-md transition-all group relative cursor-grab active:cursor-grabbing",
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-700/50 pt-2">
        ...
        <div className="flex items-center gap-1">
           {/* Avatar placeholder or user initial could go here */}
        </div>
      </div>
    </div>
  );
